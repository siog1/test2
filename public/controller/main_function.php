...
*****************************************/
function delete($base,$column,$id) {
  $command = "DELETE FROM ".$base." WHERE ".$column."=".$id;
  pdoExec($command);
}
...
function listOfMatter($selected) {
  $requete = returnPDO()->prepare('SELECT * FROM matter');
  $requete->execute();
  while ($res = $requete->fetch()) {
    $imselect = "";
    if (!empty($selected) AND $selected==$res['id_matter']) {
      $imselect == "selected";
    }
...
/*************************************
List des dates , promotions (tous les 2 ans)
*****************************************/
function dateOnMyWomb() {
  $Date = date('Y');
  for ($i = -10; $i <= 10; $i++) {
    $Date = date($Date, strtotime($i.' Years'));
    echo $Date." / ";
  }
